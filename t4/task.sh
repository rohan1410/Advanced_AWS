#!/bin/sh
echo "Creating Lambda for starting the instance"
...
aws lambda create-function --function-name pe-rmm-startec2 \
--runtime python2.7 \
...
arn1=$(aws lambda get-function-configuration --function-name pe-rmm-startec2 --region us-east-1 --query '{FunctionArn:FunctionArn}' --output text)
...
aws lambda create-function --function-name pe-rmm-stopec2 \
--runtime python2.7 \
--role arn:aws:iam::488599217855:role/FullAccess \
--handler lambda_function.lambda_handler \
...
echo "Getting ARN for the start ec2 lambda function"
arn2=$(aws lambda get-function-configuration --function-name pe-rmm-stopec2 --region us-east-1 --query '{FunctionArn:FunctionArn}' --output text)
i=0
...
do
	...
	echo "pe-rmm-startec2rule$i"
	arn3=$(aws events put-rule --name "pe-rmm-startec2rule$i" --schedule-expression "cron($start_m $start_h * * ? *)" --state "ENABLED" \
	--role-arn "arn:aws:iam::488599217855:role/FullAccess" --region "us-east-1" --query "{RuleArn:RuleArn}" --output text)
	aws lambda add-permission \
	--function-name pe-rmm-startec2 \
	--statement-id $i \
	...
	aws events put-targets --rule "pe-rmm-startec2rule$i" --targets "Id"="1","Arn"="$arn1" --region "us-east-1"

	echo "Creating Cloudwatch Rule for stopping the instance for $day"

	arn4=$(aws events put-rule --name "pe-rmm-stopec2rule$i" --schedule-expression "cron($stop_m $stop_h * * ? *)" --state "ENABLED" \
	--role-arn "arn:aws:iam::488599217855:role/FullAccess" --region "us-east-1" --query "{RuleArn:RuleArn}" --output text)

	aws lambda add-permission \
	--function-name pe-rmm-stopec2 \
	--statement-id $i \
	--action 'lambda:InvokeFunction' \
	--principal events.amazonaws.com \
	--source-arn $arn4 \
	...
	aws events put-targets --rule "pe-rmm-stopec2rule$i" --targets "Id"="1","Arn"="$arn2" --region "us-east-1"
done
